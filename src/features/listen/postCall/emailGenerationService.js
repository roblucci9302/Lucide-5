...
class EmailGenerationService {
    ...
    async generateFollowUpEmail(sessionId, options = {}) {
        try {
            ...
            // P2-2: Get user's configured AI model instead of hardcoded 'gpt-4o'
            ...
            // Add metadata - P2-2: Use actual model used
            emailData.metadata = {
                sessionId,
     ...
     * P2-1: Prompt traduit en français
     * @private
     */
    _buildEmailPrompt(structuredData, tasks, participants, template, tone, includeActionItems, includeDecisions) {
        ...
        // P2-1: Traduction du ton en français
        const toneInstructions = {
            formal: 'Utilise un langage formel et professionnel adapté au monde de l\'entreprise',
            friendly: 'Utilise un ton chaleureux et accessible tout en restant professionnel',
            technical: 'Utilise un langage clair et technique adapté à une équipe technique'
        };

        let prompt = `Tu es un assistant professionnel spécialisé dans la rédaction d'emails de suivi de réunion en français.

**Contexte de la réunion :**
- Participants : ${participantNames || 'Non spécifiés'}
- Type de template : ${template}
- Ton souhaité : ${tone}

**Résumé de la réunion :**
${structuredData.executiveSummary || 'Résumé non disponible'}

`;

        if (structuredData.keyPoints && structuredData.keyPoints.length > 0) {
            prompt += `**Points clés discutés :**
...
`;
        ...
        if (includeActionItems && tasks && tasks.length > 0) {
            prompt += `**Actions à suivre :**
${tasks.map((t, i) => `${i + 1}. ${t.task_description} (Responsable: ${t.assigned_to}, Échéance: ${t.deadline}, Priorité: ${t.priority})`).join('\n')}
...
`;
        ...
        prompt += `
**Instructions :**
1. Génère un email de suivi professionnel EN FRANÇAIS avec la structure suivante :
   - Objet (concis et clair)
   - Formule de salutation
   - Bref rappel du contexte de la réunion
   - Résumé des points clés
   ${includeDecisions ? '- Décisions prises' : ''}
   ${includeActionItems ? '- Actions avec responsables et échéances' : ''}
   - Prochaines étapes (si applicable)
   - Formule de politesse professionnelle

2. Ton : ${toneInstructions[tone] || toneInstructions.formal}

3. Formate ta réponse en JSON avec cette structure :
{
  "subject": "Objet de l'email",
  "body": "Corps de l'email en texte brut",
  "bodyHtml": "Corps de l'email en HTML avec balises <p>, <ul>, <li>"
}

4. L'email doit être concis mais complet (200-400 mots).
5. Utilise les noms des participants quand c'est approprié.
6. Les actions doivent être claires avec responsables et échéances.

Génère l'email maintenant :`;
