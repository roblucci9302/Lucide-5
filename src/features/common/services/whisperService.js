...
class WhisperService extends EventEmitter {
    ...
    async initialize() {
        ...
        try {
            ...
            const archInfo = architectureUtils.getArchitectureInfo();
            console.log(`[WhisperService] Initializing on ${archInfo.platform} (${archInfo.nativeArch})`);
            if (archInfo.isAppleSilicon) {
                console.log('[WhisperService] Detected Apple Silicon Mac');
                if (archInfo.isRosetta) {
                    console.warn('[WhisperService] Running under Rosetta 2 - consider using native arm64 binary for better performance');
                }
            }
    ...
    async installViaHomebrew() {
        ...
        // Log architecture info - Homebrew will install native binary for current arch
        const archInfo = architectureUtils.getArchitectureInfo();
        const archType = archInfo.isAppleSilicon ? 'Apple Silicon (arm64)' : 'Intel (x64)';
        console.log(`[WhisperService] Installing whisper-cpp via Homebrew for ${archType}...`);

        if (archInfo.isRosetta) {
            console.warn('[WhisperService] Note: Running under Rosetta. Homebrew should still install native arm64 binary.');
        }
        ...
        if (whisperCliPath) {
            ...
            console.log(`[WhisperService] Binary architecture: native ${archInfo.nativeArch}`);
        } else {
            const whisperPath = await this.checkCommand('whisper');
            if (whisperPath) {
                this.whisperPath = whisperPath;
    ...
    async installMacOS() {
        // whisper.cpp does not provide pre-built macOS binaries
        // Installation must be done via Homebrew which provides native arm64/x64 support
        const archInfo = architectureUtils.getArchitectureInfo();
        const archDescription = archInfo.isAppleSilicon
            ? 'Apple Silicon (arm64) - Homebrew will install native ARM binary for best performance'
            : 'Intel (x64)';

        console.log(`[WhisperService] macOS ${archDescription}`);
        console.log('[WhisperService] Pre-built macOS binaries are not available from whisper.cpp releases');
        console.log('[WhisperService] Please use Homebrew: brew install whisper-cpp');

        // Provide helpful error with architecture-specific information
        const errorMessage = archInfo.isAppleSilicon
            ? 'Pre-built macOS binaries not available. Install via Homebrew (brew install whisper-cpp) for native Apple Silicon support with optimal performance.'
            : 'Pre-built macOS binaries not available. Install via Homebrew: brew install whisper-cpp';
