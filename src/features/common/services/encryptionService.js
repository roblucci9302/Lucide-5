...
function resetSessionKey() {
    // Fix MEDIUM BUG-M12: Overwrite encryption key with zeros before clearing
    // Security best practice: prevent key from lingering in memory
    if (sessionKey) {
        // Overwrite each character of the key string with '0'
        // This doesn't guarantee memory erasure (JS strings are immutable)
        // but it's a best-effort approach to clear sensitive data
        const keyLength = sessionKey.length;
        sessionKey = '0'.repeat(keyLength);

        console.log('[EncryptionService] Encryption key securely overwritten');
    }
