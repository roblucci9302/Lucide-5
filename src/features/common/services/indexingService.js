...
class IndexingService {
     ...
     * KB-P1-3: Added progress callback for non-blocking indexation with real-time updates
    ...
    async indexDocument(documentId, content, options = {}) {
        const {
            ...
            onProgress = null // KB-P1-3: Progress callback
        } = options;
        ...
        try {
            ...
            // KB-P1-3: Report initial progress
            if (onProgress) {
                onProgress({
                    percent: 5,
                    phase: 'chunking',
                    message: `Document découpé en ${chunks.length} segments`,
            ...
            if (generateEmbeddings && chunks.length > 0) {
                ...
                for (let batchStart = 0; batchStart < chunks.length; batchStart += BATCH_SIZE) {
                    ...
                    // KB-P1-3: Report progress before embedding generation
                    if (onProgress) {
                        const progressPercent = 10 + Math.round((batchStart / chunks.length) * 80);
                        onProgress({
                            percent: progressPercent,
                            phase: 'embedding',
                            message: `Génération des embeddings... (${batchStart}/${chunks.length})`,
                            totalChunks: chunks.length,
                            processedChunks: batchStart
                        });
                    }
                ...
                // KB-P1-3: Report embedding completion
                if (onProgress) {
                    onProgress({
                        percent: 90,
                        phase: 'saving',
            ...
            // KB-P1-3: Report completion
            if (onProgress) {
                onProgress({
                    percent: 100,
                    phase: 'complete',
                    message: `Indexation terminée: ${chunkObjects.length} segments créés`,
                    totalChunks: chunkObjects.length,
                    processedChunks: chunkObjects.length
                });
            }
