...
class AutoIndexingService {
    ...
    async _extractKeyPointsWithLLM(text) {
        try {
            const providerSettings = require('./providerSettings');
            const settings = await providerSettings.getSettings();
            
            if (!settings.active_llm_provider || !settings.active_llm_model) {
                console.warn('[AutoIndexingService] No active LLM configured');
                return [];
            }

            const aiFactory = require('../ai/factory');
            const apiKey = await providerSettings.getApiKey(settings.active_llm_provider);
            
            if (!apiKey) {
                console.warn('[AutoIndexingService] No API key for active LLM provider');
                return [];
            }

            const llm = aiFactory.createLLM(settings.active_llm_provider, {
                apiKey,
