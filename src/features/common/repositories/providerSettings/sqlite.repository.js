...
// Fix CRITICAL BUG-C12/C13: SQL injection prevention - whitelist allowed column names
...
function getActiveProvider(type) {
    ...
    // Fix CRITICAL BUG-C12: Validate type against whitelist
    const column = ALLOWED_PROVIDER_TYPES[type];
    if (!column) {
        throw new Error(`Invalid provider type: ${type}. Must be 'llm' or 'stt'`);
    }
...
function setActiveProvider(provider, type) {
    ...
    // Fix CRITICAL BUG-C13: Validate type against whitelist
    const column = ALLOWED_PROVIDER_TYPES[type];
    if (!column) {
        throw new Error(`Invalid provider type: ${type}. Must be 'llm' or 'stt'`);
    }
