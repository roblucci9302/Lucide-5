...
class AnthropicProvider {
    ...
    static async validateApiKey(key) {
        return validateApiKey(key, 'anthropic');
    }
}
 ...
 * @param {object} opts - Configuration options
 * @param {string} opts.apiKey - Anthropic API key
 * @param {string} [opts.model='claude-3-haiku-20240307'] - Model name
 * @param {number} [opts.temperature=0.7] - Temperature
 * @param {number} [opts.maxTokens=4096] - Max tokens
 * @returns {object} LLM instance
 */
function createLLM({ apiKey, model = "claude-3-haiku-20240307", temperature = 0.7, maxTokens = 4096, ...config }) {
