...
contextBridge.exposeInMainWorld('api', {
  ...
  postMeeting: {
    ...
    // Phase 5: Get transcripts for a session (for Transcript tab)
    getTranscripts: (sessionId) => ipcRenderer.invoke('post-meeting:get-transcripts', sessionId)
      .catch(err => { console.error('[Preload] getTranscripts error:', err); return { success: false, transcripts: [] }; }),
  ...
  // ========================================
  // SPEAKER MANAGEMENT (Diarization)
  // ========================================
  speakers: {
    // Get list of speakers for a session
    getSessionSpeakers: (sessionId) => ipcRenderer.invoke('get-session-speakers', sessionId),

    // Rename a speaker (updates all occurrences)
    renameSpeaker: (data) => ipcRenderer.invoke('rename-speaker', data),

    // Get transcript with speaker names
    getSessionTranscript: (sessionId) => ipcRenderer.invoke('get-session-transcript', sessionId),

    // Get currently active speaker
    getCurrentSpeaker: (sessionId) => ipcRenderer.invoke('get-current-speaker', sessionId),

    // Clear session data
    clearSession: (sessionId) => ipcRenderer.invoke('clear-session-speakers', sessionId),

    // Fix HAUT #5: Listen for speaker rename events to update UI
    onSpeakerRenamed: (callback) => {
      const handler = (event, data) => callback(event, data);
      ipcRenderer.on('speaker-renamed', handler);
      return () => ipcRenderer.removeListener('speaker-renamed', handler);
    }
  }
});