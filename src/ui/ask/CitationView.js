...
export class CitationView extends LitElement {
    ...
    async handleCitationClick(citation) {
        ...
        try {
            ...
            // KB-P2-3: Use knowledge.openManager() as primary, settingsView as fallback
            if (window.api?.knowledge?.openManager) {
                window.showToast?.(`Ouverture de "${citation.document_title || 'Document'}" dans le gestionnaire...`, 'info');
                await window.api.knowledge.openManager();
            } else if (window.api?.settingsView?.openKnowledgeBaseManager) {
                window.showToast?.(`Ouverture de "${citation.document_title || 'Document'}" dans le gestionnaire...`, 'info');
