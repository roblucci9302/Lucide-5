...
export class ResponseView extends LitElement {
    ...
    /**
     * Retourne la vraie hauteur du contenu interne (pas limitée par max-height)
     * Utilisé par ListenView pour calculer la hauteur de fenêtre nécessaire
     */
    getContentHeight() {
        const container = this.shadowRoot.querySelector('.response-container');
        if (!container) return 0;
        
        const header = this.shadowRoot.querySelector('.response-header');
    ...
    setupIpcListeners() {
        ...
        window.api?.responseView?.onAiResponseReady?.((event, data) => {
            ...
            this.requestUpdate().then(() => {
                // Notifier le parent (ListenView) que les suggestions ont changé
                // pour qu'il ajuste la hauteur de la fenêtre
                this.dispatchEvent(new CustomEvent('suggestions-updated', {
                    bubbles: true,
                    composed: true,
                    detail: { count: this.suggestions.length }
                }));
            });
        });
        ...
        return html`
            <div class="response-container">
                <div class="response-header">
                    ...
                    <div>
                        <div class="response-title">Suggestions de réponse IA</div>
            ...
            <div class="copy-feedback">✓ Copié dans le presse-papiers</div>
        `;
    }
        ...
        if (this.loading) {
            return html`
                <div class="loading-state">
                    ...
                    <div class="loading-text">Génération des suggestions en cours...</div>
                </div>
            `;
        }
            ...
            return html`
                <div class="empty-state">
                    ...
                    <div class="empty-text">
                        Parlez pour obtenir des<br>suggestions de réponse IA
                    </div>
                </div>
            `;
        }
