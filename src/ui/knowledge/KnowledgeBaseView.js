...
export class KnowledgeBaseView extends LitElement {
    static styles = css`
        * {
        ...
        /* KB-UX-5: Quality indicators for embedding provider */
        .quality-indicator {
            display: inline-flex;
            align-items: center;
            gap: 6px;
    ...
    static properties = {
        ...
        embeddingStatus: { type: Object, state: true },
        // KB-P3-2: Separate loading state for stats
        statsLoading: { type: Boolean, state: true }
    };
    ...
    static icons = {
        ...
        // KB-UX-5: Embedding provider icon
    ...
    connectedCallback() {
        ...
        this.loadDocuments();
        this.loadStats();
        this.loadEmbeddingStatus(); // KB-UX-5: Load embedding provider status
        // Keyboard shortcuts
    ...
    _handleKeyDown(e) {
        ...
        // KB-P3-3: Ctrl+U to upload document
    ...
    /**
     * KB-P2-1: Read file as base64 without blocking the UI thread
     * Uses FileReader API which is async and handles large files efficiently
     * For very large files, uses chunked processing to avoid memory issues
     * @param {File} file - File object to read
     * @param {Function} onProgress - Progress callback (0-100)
     * @returns {Promise<string>} Base64 encoded string (without data: prefix)
     */
    _readFileAsBase64NonBlocking(file, onProgress = () => {}) {
        return new Promise((resolve, reject) => {
            const CHUNK_SIZE = 1024 * 1024; // 1MB chunks for progress reporting

            // For smaller files, use simple FileReader
            if (file.size < CHUNK_SIZE * 5) {
                ...
                // KB-P2-1: For large files, use chunked ArrayBuffer reading
                // This provides better progress tracking and avoids memory spikes
                this._readLargeFileAsBase64(file, CHUNK_SIZE, onProgress)
                    .then(resolve)
                    .catch(reject);
            }
        });
    ...
    /**
     * KB-P2-1: Read large file in chunks and convert to base64
     * Uses async/await with setTimeout to yield to the event loop
     * @param {File} file - File to read
     * @param {number} chunkSize - Size of each chunk
     * @param {Function} onProgress - Progress callback
     * @returns {Promise<string>} Base64 encoded string
     */
    async _readLargeFileAsBase64(file, chunkSize, onProgress) {
        ...
        const ENCODE_CHUNK_SIZE = 32768; // Process 32KB at a time for base64 encoding
        let binary = '';

        for (let i = 0; i < combined.length; i += ENCODE_CHUNK_SIZE) {
            const slice = combined.slice(i, Math.min(i + ENCODE_CHUNK_SIZE, combined.length));
            binary += String.fromCharCode.apply(null, slice);

            // Yield periodically during encoding
            if (i % (ENCODE_CHUNK_SIZE * 10) === 0 && i > 0) {
                await new Promise(resolve => setTimeout(resolve, 0));
            }
        }
    ...
    async loadStats() {
        // KB-P3-2: Track loading state for skeleton display
        this.statsLoading = true;
        try {
    ...
    // KB-UX-5: Load embedding provider status
    async loadEmbeddingStatus() {
        try {
            const result = await window.api.knowledge.getEmbeddingStatus();
            if (result) {
                this.embeddingStatus = result;
            }
        } catch (error) {
            console.error('[KnowledgeBaseView] Error loading embedding status:', error);
        }
    }

    // KB-UX-5: Get display label for embedding provider
    _getProviderLabel(provider) {
        switch (provider) {
            case 'openai': return 'OpenAI';
            case 'openai-fallback': return 'Dégradé';
            case 'mock': return 'Test';
            default: return 'Inconnu';
        }
    }

    // KB-UX-5: Get quality indicator class
    _getQualityClass(quality) {
        switch (quality) {
            case 'semantic': return 'quality-good';
            case 'basic': return 'quality-degraded';
    ...
    async _uploadFile(file) {
        ...
        try {
            ...
            // KB-P2-1: Non-blocking progress update
            ...
            // KB-P2-1: Use FileReader with async/Promise to avoid blocking UI
            // This is more efficient than manual ArrayBuffer -> base64 conversion
            const base64 = await this._readFileAsBase64NonBlocking(file, (percent) => {
                // Scale progress from 10-50 during file reading
                updateProgress(10 + Math.round(percent * 0.4));
            });
    ...
    async handleDeleteDocument(documentId, title) {
        // KB-UX-2: Enriched delete confirmation with document details
        const doc = this.documents.find(d => d.id === documentId);

        // Build detailed confirmation message
        let detailsMessage = 'Cette action est irréversible.\n\n';

        if (doc) {
            const details = [];
            if (doc.file_type) {
                details.push(`• Type : ${doc.file_type.toUpperCase()}`);
            }
            if (doc.file_size) {
                details.push(`• Taille : ${this.formatSize(doc.file_size)}`);
            }
            if (doc.chunk_count) {
                details.push(`• Chunks indexés : ${doc.chunk_count}`);
            }

            if (details.length > 0) {
                detailsMessage += 'Détails du document :\n' + details.join('\n') + '\n\n';
            }
        }

        detailsMessage += '⚠️ Le document et tous ses chunks d\'indexation seront définitivement supprimés de la base de connaissances.';
    ...
    /**
     * KB-P0-2: Sanitize URL to prevent javascript: and data: XSS attacks
     * Only allows http, https, mailto, and # (anchor) protocols
     */
    _sanitizeUrl(url) {
        if (!url) return '#';
        ...
        if (trimmed.startsWith('javascript:') ||
            ...
            console.warn('[KnowledgeBaseView] Blocked dangerous URL:', url.substring(0, 50));
            return '#blocked';
        }
        // Assume relative URL is safe
        return url;
    }

    /**
     * KB-P0-2: Sanitize HTML output using DOM-based approach
     * Whitelist of allowed tags and attributes to prevent XSS
     */
    _sanitizeHtml(html) {
                ...
                if (child.nodeType === Node.ELEMENT_NODE) {
                    ...
                    if (!ALLOWED_TAGS.has(tagName)) {
                        ...
                        if (tagName === 'a') {
                            ...
                            child.setAttribute('href', this._sanitizeUrl(href));
    ...
    _renderMarkdown(content) {
        ...
        let html = content
            ...
            // Links - KB-P0-2: Use sanitized URL
        ...
        // KB-P0-2: Final sanitization pass
    ...
    // KB-P3-2: Skeleton loading for stats with 4 cards (including embedding provider)
    ...
    render() {
        return html`
            ...
            >
                ...
                <!-- Stats - KB-P3-2: Show skeleton while loading -->
                ${this.statsLoading ? this._renderSkeletonStats() : html`
                    <div class="stats-section">
                        <div class="stat-card">
                        ...
                        <!-- KB-UX-5: Embedding provider status -->
                        <div class="stat-card">
                            <span class="stat-icon-wrapper">${this._icon('brain', 'icon-lg')}</span>
                            <div class="stat-label">Recherche sémantique</div>
                            <div class="stat-value-small">${this._getProviderLabel(this.embeddingStatus.provider)}</div>
                            <span class="quality-indicator ${this._getQualityClass(this.embeddingStatus.quality)}">
                                ${this.embeddingStatus.quality === 'semantic' ? '● Optimale' :
                                  this.embeddingStatus.quality === 'basic' ? '○ Limitée' : '○ Inconnue'}
                            </span>
                        </div>
                    </div>
                `}
                ...
                <div class="search-section" role="search">
                    ...
                    <button
                        ...
                        aria-label="Ajouter un nouveau document (Ctrl+U)"
                        title="Ajouter un document (Ctrl+U)"
                    >
                        ${this.uploading
