...
export class MainHeader extends TranslationMixin(LitElement) {
    ...
    _getListenButtonText(status) {
        switch (status) {
            ...
            case 'inSession'   : return this.t('header.stop');  // "Arrêter" to stop the session
    ...
    connectedCallback() {
        ...
        if (window.api) {
            ...
            this._sessionStateTextListener = (event, { success, action }) => {
                if (success) {
                    // Handle explicit action from backend if provided
                    if (action === 'stopped') {
                        this.listenSessionStatus = 'afterSession';
                    } else {
                        // Simplified state transitions: Listen → Stop → Done
                        this.listenSessionStatus = ({
                            beforeSession: 'inSession',       // Start listening
                            inSession: 'afterSession',        // Stop session (will be handled by backend)
                            afterSession: 'beforeSession',    // Reset to start
                        })[this.listenSessionStatus] || 'beforeSession';
                    }
                } else {
                    this.listenSessionStatus = 'beforeSession';
    ...
    render() {
        ...
        const showStopIcon = this.listenSessionStatus === 'inSession';
        const showDoneIcon = this.listenSessionStatus === 'afterSession';
                    ...
                    ${this.isTogglingSession
                        ...
                        : html`
                            ...
                            <div class="listen-icon">
                                ${showStopIcon
                                    ? html`
                                        <!-- Stop icon (square) - shown when session is active -->
