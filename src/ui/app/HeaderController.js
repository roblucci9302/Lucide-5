...
class HeaderTransitionManager {
    constructor() {
        ...
        // Méthodes de callback pour WelcomeHeader
    ...
    async _bootstrap() {
        ...
        try {
            if (window.api && window.api.common) {
                ...
                console.log('[HeaderController] No window.api available, showing welcome header');
                await this._resizeForWelcome();
                this.ensureHeader('welcome');
            }
        } catch (error) {
            // CRITICAL: If bootstrap fails, still show something to the user
            console.error('[HeaderController] Bootstrap failed, falling back to welcome header:', error);
            try {
                await this._resizeForWelcome();
            } catch (resizeError) {
                console.error('[HeaderController] Resize also failed:', resizeError);
            }
            this.ensureHeader('welcome');
        }
    }
    ...
    async handleStateUpdate(userState) {
        try {
            ...
            if (!window.api || !window.api.apiKeyHeader) {
                console.warn('[HeaderController] API not available, showing welcome header');
                await this._resizeForWelcome();
                this.ensureHeader('welcome');
                return;
            }
            ...
            console.error('[HeaderController] handleStateUpdate error, showing welcome header:', error);
            try {
                await this._resizeForWelcome();
            } catch (resizeError) {
                console.error('[HeaderController] Resize also failed:', resizeError);
    ...
    async handleApiKeyOption() {
        ...
        this.ensureHeader('apikey');
        // Configurer le callback de retour pour ApiKeyHeader
        if (this.apiKeyHeader) {
            this.apiKeyHeader.backCallback = () => this.transitionToWelcomeHeader();
        }
    }
