...
export class ApiKeyHeader extends TranslationMixin(LitElement) {
    ...
    constructor() {
        ...
        this.providers = { llm: [], stt: [] }; // Initialisation
        // Ollama related
    ...
    async loadProviderConfig() {
        ...
        try {
            ...
            for (const id in config) {
                // Ne pas afficher les providers virtuels comme 'openai-lucide' dans l'UI
                if (id.includes('-lucide')) continue;
    ...
    async loadModelSuggestions() {
        ...
        try {
            ...
            if (result?.success) {
                ...
                // Sélectionner le modèle par défaut (premier modèle installé)
    ...
    async ensureOllamaReadyWithUI() {
        ...
        const progressHandler = (event, data) => {
            // Traiter uniquement les événements de progression Ollama des événements LocalAI unifiés
        ...
        const completionHandler = async (event, data) => {
            // Traiter uniquement les événements de complétion Ollama des événements LocalAI unifiés
            if (data.service !== 'ollama') return;
            ...
            // L'événement installation-complete signifie le succès
            await this._handleOllamaSetupCompletion(true);
        };

        // Utiliser les événements LocalAI unifiés
    ...
    async installModel(modelName) {
        ...
        try {
            ...
            // Configuration du suivi de progression - Utilisation des événements LocalAI unifiés
            window.api.apiKeyHeader.onLocalAIProgress(progressHandler);
    ...
    async downloadWhisperModel(modelId) {
        ...
        try {
            // Configuration de l'écouteur de progression robuste - Utilisation des événements LocalAI unifiés
            progressHandler = (event, data) => {
    ...
    openPrivacyPolicy() {
        ...
        if (window.api?.common) {
            window.api.common.openExternal('https://lucide.app/privacy-policy');
        }
    }
