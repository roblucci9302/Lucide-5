...
class SmoothMovementManager {
    ...
    _isWindowValid(win) {
        if (!win || win.isDestroyed()) {
            // Nettoyer le timer de cette fenêtre s'il existe
    ...
    animateWindow(win, targetX, targetY, options = {}) {
        ...
        const step = () => {
            ...
            if (p < 1) {
                ...
                // Fix CRITICAL BUG-C9: Removed undefined layoutManager.updateLayout() call
                // Layout updates are handled by windowManager.js via updateChildWindowLayouts()
                if (onComplete) {
                    onComplete();
                }
            }
    ...
    destroy() {
        ...
        // Fix CRITICAL BUG-C10: Clear all animation timers to prevent memory leak
