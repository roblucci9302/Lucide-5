...
async function handleWindowVisibilityRequest(windowPool, layoutManager, movementManager, name, shouldBeVisible) {
    try {
        ...
        // FIX: Recreate window if destroyed and visibility is requested
        if ((!win || win.isDestroyed()) && shouldBeVisible) {
            console.warn(`[WindowManager] Window '${name}' not found or destroyed. Recreating...`);
            
            // Remove destroyed window from pool first
            if (win && win.isDestroyed()) {
                windowPool.delete(name);
            }
            
            // Recreate the window
            if (name === 'post-meeting' || name === 'settings' || name === 'shortcut-settings' || name === 'listen' || name === 'ask') {
                const header = windowPool.get('header');
                if (header && !header.isDestroyed()) {
                    ...
                    if (!win || win.isDestroyed()) {
                        console.error(`[WindowManager] Failed to recreate window '${name}'`);
                        return;
                    }
                } else {
                    console.error(`[WindowManager] Cannot recreate '${name}' - header not available`);
                    return;
                }
            } else {
                console.warn(`[WindowManager] Window '${name}' recreation not supported`);
                return;
            }
